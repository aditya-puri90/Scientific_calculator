...
1. The user taps buttons to build an **expression** (e.g., `sin(30)+5×2`)
2. Symbols like `×` and `÷` are internally replaced with `*` and `/` before parsing
3. The `math_expressions` package parses and evaluates the expression
4. The result is displayed with up to 4 decimal places (trailing zeros removed)
5. After evaluation, the result is loaded back into the expression for further chaining
